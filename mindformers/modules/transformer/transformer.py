...
class FeedForward(Cell):
    ...
    def __init__(self, hidden_size,
        ...
        if _get_parallel_mode() in (ParallelMode.AUTO_PARALLEL,):
            ...
            self.mapping = nn.Dense(input_size,output_size).to_float(mstype.float32)

            # Project back to hidden_size
            self.projection = nn.Dense(output_size,input_size).to_float(mstype.float32)
            if expert_num > 1:
    ...
    def construct(self, x):
        ...
        # print("ffn start", x[256+10, :10])
        # print("ffn start", x[256+20, :10])
        # print("self.mapping.weight[10, :10]", self.mapping.weight.shape, self.mapping.weight[10, :10])
        ...
        # print("ffn end", output[256+10, :10])
        # print("ffn end", output[256+20, :10])
...
class AttentionMask(Cell):
    ...
    def __init__(self, seq_length, parallel_config=default_dpmp_config, compute_dtype=mstype.float32):
...
class MultiHeadAttention(Cell):
    ...
    def __init__(self, batch_size,
        ...
        if _get_parallel_mode() in (ParallelMode.AUTO_PARALLEL,):
            ...
            self.is_first_iteration = True
            # Output layer
            self.projection = nn.Dense(hidden_size,hidden_size)
            ...
            self.projection = nn.Dense(hidden_size,
                                     hidden_size).to_float(mstype.float32)
            self.projection.bias.parallel_optimizer = False
...
class TransformerEncoderLayer(Cell):
    ...
    def construct(self, x, input_mask=None, init_reset=True, batch_valid_length=None):
        ...
        # print("after attention:", output_x[256+10, :10])
        # print("after attention:", output_x[256+20, :10])
        ...
        # print("after ffn:", output[1, 10, :10])
        # print("after ffn:", output[1, 20, :10])
...
class TransformerEncoder(Cell):
    ...
    def construct(self, hidden_states, attention_mask, init_reset=True, batch_valid_length=None):
        ...
        for i in range(self.num_layers):
            # print(hidden_states[1, 10, :].sum(), hidden_states[1, 20, :].sum())
