...
class BertModel(BertPreTrainedModel):
    ...
    def construct(self, input_ids, token_type_ids, input_mask):
        ...
        # print(embedding_output[1, 10, :].sum(), embedding_output[1, 20, :].sum())
        # exit()
...
class BertForMultipleChoice(BertPreTrainedModel):
    ...
    def __init__(self, config=BertConfig()):
        ...
        self.lambda_ratio = config.num_labels
        self.bert = BertModel(config, config.use_one_hot_embeddings)
        ...
        self.cat = P.Concat(1)
        self.cat_0 = P.Concat(0)
        self.log_softmax = P.LogSoftmax()

    def KL(self, p1, p2, m):
        # return torch.sum(p1 * (torch.log(p1 + 1e-6) - torch.log(p2 + 1e-6)))
        res = p1 * (self.log(p1 + 1e-6) - self.log(p2 + 1e-6))
        res = res - res * m
        return res.sum(-1)

    def construct(self, sentences, sentences_am, sentences_type, sentences_pair, sentences_pair_am, sentences_pair_type, sentences_neg, sentences_neg_am, sentences_neg_type):
        """Get Training Loss or Logits"""

        doc_num, sent_num, seq_num = sentences.shape
        doc_num_1, sent_num_1, seq_num_1 = sentences_pair.shape
        doc_num_2, sent_num_2, seq_num_2 = sentences_neg.shape

        all_input = self.cat_0([sentences.view(-1, seq_num), sentences_pair.view(-1, seq_num), sentences_neg.view(-1, seq_num)])
        all_type = self.cat_0([sentences_type.view(-1, seq_num), sentences_pair_type.view(-1, seq_num), sentences_neg_type.view(-1, seq_num)])
        all_am = self.cat_0([sentences_am.view(-1, seq_num), sentences_pair_am.view(-1, seq_num), sentences_neg_am.view(-1, seq_num)])

        _, pooled_output, _ = self.bert(all_input, all_type, all_am)
        all_logits = self.classifier(pooled_output)
        sentences_logits = all_logits[:doc_num*sent_num, :].view(doc_num, sent_num)

        sentences_pair_logits = all_logits[doc_num*sent_num:doc_num*sent_num+doc_num_1*sent_num_1, :].view(doc_num_1, sent_num_1)

        mask_sentences = sentences[:, :, 0] == 0
        mask_sentences_pair = sentences_pair[:, :, 0] == 0

        # Minus 10000 for the position where masked to exclude them from softmax
        sentences_logits = sentences_logits + mask_sentences * -10000
        sentences_pair_logits = sentences_pair_logits + mask_sentences_pair * -10000

        sentences_p = self.softmax(sentences_logits)
        sentences_pair_p = self.softmax(sentences_pair_logits)

        part_1 = self.KL(sentences_p, (sentences_p + sentences_pair_p) / 2, mask_sentences)
        part_2 = self.KL(sentences_pair_p, (sentences_p + sentences_pair_p) / 2, mask_sentences_pair)
        loss = ((part_1+part_2) / 2).sum() * self.lambda_ratio #-0.2

        # matching
        sentences_neg_logits = all_logits[-doc_num_2*sent_num_2:, :].view(doc_num_2, sent_num_2)

        max_value = self.reduce_max(sentences_logits, -1)
        tmp_res = self.cat([max_value, sentences_neg_logits])

        # tmp_res = self.log_softmax(tmp_res)[:, 0] * -1
        tmp_res = self.cross_entropy_loss(tmp_res, (tmp_res[:, 0] == 1000).int())
        loss = loss + tmp_res
...
@MindFormerRegister.register(MindFormerModuleType.MODELS)
class BertForMultipleChoiceEval(BertPreTrainedModel):
    _support_list = MindFormerBook.get_model_support_list()['txtcls']['bert']

    def __init__(self, config=BertConfig()):
        super(BertForMultipleChoiceEval, self).__init__(config)
        self.num_labels = config.num_labels
        self.bert = BertModel(config, config.use_one_hot_embeddings)
    ...
    def construct(self, sentences, sentences_am, sentences_type):
        """Get Training Loss or Logits"""

        doc_num, sent_num, seq_num = sentences.shape

        _, pooled_output, _ = self.bert(sentences.view(-1, seq_num), sentences_type.view(-1, seq_num), sentences_type.view(-1, seq_num))
        all_logits = self.classifier(pooled_output)
        tmp_res = all_logits.view(doc_num, sent_num)

        loss = self.cross_entropy_loss(tmp_res, (tmp_res[:, 0] == 1000).int())

        if self.training:
            output = loss
        else:
            output = (tmp_res, (tmp_res[:, 0] == 1000).int())
